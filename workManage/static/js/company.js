$(function(){
    ...
    $("#btn_delete").on('click',function(){
    	var selRow = $('#company_info').bootstrapTable('getSelections');
    	...
    	$.ajax({
            ...
            success: function(data){
            	if ( data.message == 'success' ){
                    ...
                    $('#company_info').bootstrapTable('refresh');
                }
            }
         });
    ...
    $("#btn_edit").on('click',function(){
    	...
    	var address = selRow[0].address;
    	var headPeople = selRow[0].headPeople;
    	var tel = selRow[0].tel;
    	var remark = selRow[0].remark;
    	
    	$("input[name='id']").val(id);
    	$(":text[name='name']").val(name);
    	$(":text[name='address']").val(address);
    	$(":text[name='tel']").val(tel);
    	$("input[name='headPeople']").val(headPeople);
    	$(":text[name='remark']").val(remark);
    	$('#myModal').modal('show');
    });
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#company_info').bootstrapTable({
            url: '/getCompanyInfo',         		//请求后台的URL（*）
            method: 'get',                      //请求方式（*）
